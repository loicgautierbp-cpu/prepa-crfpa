...
const PROCEDURES = ['proc-civile', 'proc-penale', 'proc-admin'];
const procedures = SUBJECTS.filter(s => PROCEDURES.includes(s.id));
...
const EXERCISE_TYPES = [
  {
    ...
    description: 'Résolvez un cas concret : compétence, délais, recevabilité, voies de recours, exécution des décisions.',
    duree: '2h',
    difficulty: 'Classique',
    tags: ['Qualification', 'Compétence', 'Méthode IRAC'],
    icon: (
  ...
  },
  {
    ...
    description: 'Conseillez un client sur la stratégie procédurale : juridiction, recours possibles, délais, chances de succès.',
    duree: '2h',
    difficulty: 'Pratique',
    tags: ['Stratégie', 'Analyse des options', 'Recommandation'],
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
...
export default function ProcedureClient({ embedded = false }) {
  const { user } = useAuth();
  const { isPremium } = usePremium();
  const { seconds, formatted, start, stop, reset } = useTimer({ mode: 'up' });

  // Steps: 'procedure' | 'type' | 'exercise' | 'correction'
  ...
  const handleStartExercise = async (type) => {
    ...
    try {
      const res = await fetch('/api/generate-procedure', {
        ...
        body: JSON.stringify({
          mode: 'generate',
          specialite: selectedProcedure.id,
          type,
        }),
      });
  ...
  const procColors = selectedProcedure ? SUBJECT_COLORS[selectedProcedure.color] || SUBJECT_COLORS.primary : null;
      ...
      <div className={embedded ? 'max-w-4xl mx-auto' : 'max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12'}>
        ...
        <div className="mb-10">
          {embedded && step === 'procedure' ? (
            <>
              ...
              <p className="text-sm text-gray-500">2 heures — Coefficient 2 — Procédure civile, pénale ou administrative</p>
            </>
            ...
            <>
              ...
              >
                {step === 'exercise' && selectedProcedure?.name}
                {step === 'correction' && "Correction"}
              </h1>
              <p className="text-gray-500 text-sm sm:text-base">
                {step === 'exercise' && (
                  selectedType === 'cas-pratique'
                    ? "Cas pratique — 2 heures — Coefficient 2"
                    : "Consultation juridique — 2 heures — Coefficient 2"
                )}
                {step === 'correction' && `${selectedProcedure?.name} — ${selectedType === 'cas-pratique' ? 'Cas pratique' : 'Consultation juridique'}`}
              </p>
            </>
          )}
        </div>
        ...
        {step === 'procedure' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {procedures.map((proc) => {
              const colors = SUBJECT_COLORS[proc.color] || SUBJECT_COLORS.primary;
              const iconData = SUBJECT_ICONS[proc.id];
              return (
                <button
                  key={proc.id}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 leading-relaxed">
                      Le cas pratique vérifie votre capacité à qualifier les faits et appliquer les règles procédurales. La consultation juridique, elle, évalue votre sens stratégique. Alternez les deux exercices pour être prêt le jour J.
                    </p>
                  </div>
                </div>
              </div>
        ...
        {step === 'exercise' && (
          <div className="space-y-6">
            ...
            {!subject && isGenerating && (
              <GenerationLoader
                title={selectedType === 'cas-pratique' ? 'Génération de votre cas pratique' : 'Génération de votre consultation'}
                subtitle={`${selectedProcedure?.name} · ${selectedType === 'cas-pratique' ? 'Cas pratique' : 'Consultation'}`}
                accent="#b91c1c"
              />
            )}

            {/* Fallback si la génération échoue ou pas encore lancée */}
            {!subject && !isGenerating && (
              <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm text-center">
                <h3 className="text-lg font-bold text-gray-900 mb-2">
                  {selectedType === 'cas-pratique' ? 'Générer un cas pratique' : 'Générer une consultation'}
                </h3>
                <p className="text-sm text-gray-500 mb-6 max-w-md mx-auto">
                  L'IA va générer un sujet de niveau CRFPA en {selectedProcedure?.name}. Vous disposerez de 2 heures pour rédiger votre copie.
                </p>
                <button
                  onClick={handleGenerate}
                  className="px-8 py-3 bg-accent-500 text-white font-semibold rounded-xl hover:bg-accent-600 transition-colors shadow-lg shadow-accent-500/20"
                >
                  Générer un sujet
                </button>
            ...
            {subject && (
              <>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm flex items-center justify-between">
                  ...
                  {procColors && (
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${procColors.badge}`}>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-primary-100 text-primary-700">
                      {selectedType === 'cas-pratique' ? 'Cas pratique' : 'Consultation juridique'}
                    </span>
                    {subject.theme && (
                  ...
                  {/* Consultation: situation + client question */}
                  {selectedType === 'consultation' && (
                    <>
                      <div className="bg-gray-50 rounded-lg p-5 border border-gray-200 mb-4">
                        <p className="text-sm font-semibold text-gray-700 mb-2">Situation :</p>
                        <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{subject.situation}</p>
                      </div>
                      {subject.clientQuestion && (
                        <div className="bg-primary-50 border-l-4 border-primary-500 rounded-r-lg p-4 mb-4">
                          <p className="text-sm font-semibold text-primary-800 mb-1">Question du client :</p>
                          <p className="text-sm text-primary-800 leading-relaxed">{subject.clientQuestion}</p>
                        </div>
                      )}
                    </>
                  )}
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  <textarea
                    ...
                    placeholder={
                      selectedType === 'cas-pratique'
                        ? "Rédigez votre cas pratique ici. Pour chaque question : identifiez la règle de procédure applicable (article CPC/CPP/CJA), vérifiez les délais et conditions de recevabilité, appliquez aux faits et concluez..."
                        : "Rédigez votre consultation ici. Analysez la situation procédurale du client, identifiez les options (juridiction compétente, recours possibles, délais), recommandez une stratégie motivée..."
                    }
