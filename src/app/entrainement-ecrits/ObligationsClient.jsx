            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {EXERCISE_TYPES.map((type) => {
                ...
                return (
                  <div
                    key={type.id}
                    className={`lift group relative bg-white rounded-2xl border border-slate-200 border-t-4 ${type.color.border} p-6 flex flex-col transition-all hover:shadow-xl ${isDisabled ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                    {/* Badge difficulté */}
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 w-full px-4 py-3 bg-[#b91c1c] text-white text-sm font-semibold rounded-xl hover:bg-[#991b1b] transition-colors disabled:opacity-60 disabled:cursor-wait shadow-sm shadow-[#b91c1c]/20"
                    >
                      {isLoading ? (
                        <>
